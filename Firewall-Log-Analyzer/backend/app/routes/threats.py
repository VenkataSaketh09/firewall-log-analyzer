...
def _filter_by_severity(items, severity: Optional[str]):
    ...
    if not sev:
        return items
    # Make severity filter inclusive: HIGH includes CRITICAL and HIGH, MEDIUM includes all above, etc.
    severity_order = ["LOW", "MEDIUM", "HIGH", "CRITICAL"]
    if sev not in severity_order:
        return items
    sev_index = severity_order.index(sev)
    # Include the selected severity and all higher severities
    allowed_severities = severity_order[sev_index:]
    return [i for i in items if (getattr(i, "severity", None) or "").upper() in allowed_severities]
